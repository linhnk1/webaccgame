...
require 'KHOAKILLALL_SYSTEM/khoa_config.php'; // website config 
...
if($user['username'] == 'kunkeypr'){
    exit('Mày định reg acc tên kunkeypr để chiếm quyền admin chứ gì? Thôi mày cút dùm bố');
}
    ...
    public function settings($option) {
        ...
        $_background = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_background'"));
        ...
        $_topnap = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_topnap'"));
        ...
        switch ($option) {
            ...
            case 'napthe':
                ...
                break;
            case 'topnap':
                return json_decode($_topnap['value'], true);
                break;
            case 'brick':
...
return $id;
   ...
   public function check_user_fb_register($idfb) {
    ...
    public function is_admin() {
        $user = $this->user();
        $rule = $user['admin'];
        if($rule == 1412) {    // set admin = 1412 (Kaito Kid :3)
            return true;
        }else {
            return false;
        }
   ...
   public function vongquaykimcuong_image($id, $type) {
    ...
    public function vongquayquanhuy_image($id, $type) {

        $path = $_SERVER["DOCUMENT_ROOT"]."/upload/vongquay_quanhuy/".$type;
                  ...
                  if ($opendirectory = opendir($path)){
                    while (($file = readdir($opendirectory)) !== false){
                        ...
                        if ($file != "." && $file != "..") {
                            $arr = explode(".", $file);
                            if($arr[0] == $id) {
                               return '/upload/vongquay_quanhuy/'.$type.'/'.$file;
                            }
                        }
                    }
                    closedir($opendirectory);
    ...
    public function vongquayquanhuy_gift($id, $item, $type=null) {

       $res = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `vongquay_quanhuy_gift` WHERE `id_vongquay`='".$id."'"));
       $prefix = 'item_';
       $json = json_decode($res[$prefix.$item], true);
    ...
    public function count_acc_ff($type) {
        ...
        switch ($type) {
            case 'conlai':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `lienquan` WHERE `status`='1' AND `nguoimua`='null' ");
        $row = mysqli_num_rows($result);
                break;

            case 'daban':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `lienquan` WHERE `status`='0'");
        $row = mysqli_num_rows($result);
                break;
        }
